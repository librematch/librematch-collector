...
export class LobbyResolver {
    ...
    async reviveLobbies(matches: any) {
    ...
    async lobbiesUpdatedSub() {
        console.log('lobbiesUpdatedSub');

        const base = {};

        const observer = observe(base);
        const lobbies = await this.cache.get(CACHE_LOBBIES) as any[] || [];

        const matches = await this.reviveLobbies(lobbies);
        ...
        return prefillIterator(
            mapIterator(
                this.pubSub.asyncIterator(PUBSUB_LOBBIES),
                async (lobbies: any) => {
                    const matches = await this.reviveLobbies(lobbies);
